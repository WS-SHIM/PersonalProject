...
<script
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=3991ef6aea5096bb617fa9b3c7a2a715&libraries=services"></script>
</head>
<body>
	<div class="container">
		<jsp:include page="../views/include/nav.jsp" />
		<div id="map" style="width: 1135px; height: 350px;"></div>
		<!-- 위치 정보 가져오는 버튼 -->
		<button type="button" class="btn btn-md btn-primary"
			onclick="getCurrentPosBtn()">내위치</button>
		<button type="button" class="btn btn-md btn-primary"
			onclick="getCurrentPosByPickBtn()">직접입력</button>
		<input type="text" class="form-control" id="keyword"
			placeholder="키워드(ex 국밥, 햄버거)">
		<button type="button" class="btn btn-md btn-primary"
			onclick="keywordSearch()">검색</button>
		<div class="radio">
			<div>최대 검색 거리 선택</div>
			<label> <input type="radio" name="distance" value="500"
				checked>500M
			</label> <label> <input type="radio" name="distance" value="1000">1000M
			</label> <label> <input type="radio" name="distance" value="2000">2000M
			</label>
		</div>
	...
	<script>
		...
		mapOption = {
			center : new kakao.maps.LatLng(35.8217994571359, 128.631179366631), // 지도의 중심좌표
			level : 3, // 지도의 확대 레벨
		...
		function getCurrentPosByPickBtn() {
			var userInput = prompt('대략적인 위치를 입력해주세요!(대구시 수성구 대흥동)');
		...
		function keywordSearch() {
			...
			function naverSearch(placeName) {
				...
				// 네이버 검색
				return 'https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query='
						+ joinedArr;
			}
